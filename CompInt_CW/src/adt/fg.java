...
public class fg {
		...
		public fg(String Expression, EXPRESSIONTYPE Type)
		{
			switch (Type)
			{
				case Prefix:
		...
		private ExpressionNode ConstructPrefixExpressionTree(String PrefixExpression)
		{
			...
			{
				if (fg.OPERATIONS.indexOf(Term) == -1)
				{
					try
					{
				...
				{
					Nodes.add(new ExpressionNode(fg.GetOperation(fg.OPERATIONS.indexOf(Term))));
				}
			}
		...
		private ExpressionNode ConstructInfixExpressionTree(String InfixExpression)
		{
			...
			{
				if (fg.OPERATIONS.indexOf(InfixExpression.charAt(i)) != -1)
				{
				...
				for (int i = 0; i < Nodes.size(); ++i)
				{
					switch (Nodes.get(i).toString().charAt(0))
					{
						...
						case '^':
							...
							while ((OperationStack.size() > 0) && (OperationStack.get(0).toString().equals("(") == false) && (fg.Precedence(Nodes.get(i).toString()) <= fg.Precedence(OperationStack.get(0).toString())))
							{
								ValueStack.add(0, OperationStack.remove(0));
								ValueStack.get(0).SetChildren(ValueStack.remove(2), ValueStack.remove(1));
							}
		...
		private class ExpressionNode
		{
			...
			{
				return String.valueOf(fg.OPERATIONS.charAt(this.Operation.ordinal()));
			}
			public String GetPrefixExpression()
			{
